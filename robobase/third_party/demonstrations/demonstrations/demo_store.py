...
def strip_gain_segment(demos_dir: Path) -> Path:
    s = demos_dir.as_posix()
    segment = "H1_HIGH_GAIN/"
    if segment in s:
        s = s.replace(segment, "")
        return Path(s)
    return demos_dir
